...
class MongoCRUD(icrud.ICRUD):
    ...
    def inject(self, module, field, id_key, deleteCascade=False, id_key_def=None):
        ...
        def inject_on_insert(_id, obj):
            if id_key_def:
                self.db.update({id_key: obj.get(id_key_def)}, {
                               '$set': {field: obj}}, multi=True)
            else:
                self.db.update({id_key: _id}, {
                               '$set': {field: obj}}, multi=True)
        ...
        def self_inject_on_upsert(_id, obj, old):
            ...
            if obj[id_key]:
                if id_key_def:
                    self.db.update({id_key_def: obj[id_key]}, {
                                   '$set': {field: module.get({id_key_def: obj[id_key]})}})
                else:
                    self.db.update(
                        {'_id': _id}, {'$set': {field: module.get(obj[id_key])}})
    ...
    def aggregate(self, count=True, index=None, size=None, conditions=None, groups=None, aggregate=None, order=None,
        ...
        if aggregate:
            for a in aggregate:
                ...
                f = a.get('Field', '') if a.get(
                    'Alias', '') == '' else a.get('Alias', '')
        ...
        # 展开的数组字段
        if unwind:
            p_unwind['$unwind'] = '$' + unwind

        if index > 1 and (size is not None):
            p_skip['$skip'] = (index - 1) * size
        ...
        if p_m['$match']:
            pipeline.append(p_m)
        if p_unwind['$unwind']:
            pipeline.append(p_unwind)
        if p_g['$group']['_id'] or len(p_g['$group'].items()) > 1:
            pipeline.append(p_g)
        if p_f['$project']:
            pipeline.append(p_f)
        if p_s['$sort']:
            pipeline.append(p_s)
        if p_skip['$skip']:
            pipeline.append(p_skip)
        if p_limit['$limit']:
            pipeline.append(p_limit)
        ...
        def assemble(rs, count):
            ...
            c = 0
            if count:
                c = rs._CommandCursor__collection.count()
